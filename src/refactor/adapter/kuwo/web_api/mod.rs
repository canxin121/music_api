...
mod kuwo_web_api_test {
    use crate::refactor::adapter::kuwo::web_api::{
        album::get_kuwo_music_album, info::get_music_info, lyric::get_kuwo_lyric,
    ...
    async fn test_get_album() {
        ...
        let result = get_kuwo_music_album(album_id, 1, 30).await.unwrap();
        println!("{:?}", result);
    }
